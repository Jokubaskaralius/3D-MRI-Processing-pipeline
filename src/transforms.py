__all__ = [
    'FeatureScaling', 'Crop', 'Resize', 'GaussianBlur',
...
class TransformManager:
    ...
    Map a transform function to a transform function string name

    config: config.json project parameter object
    '''
...
class FeatureScaling():
    ...
    Perform feature scaling on image data. Reduces the complexity of further quantitative analysis.
    Easier non-convex, convex optimizations due to localized loss function.

    sample: image data
    method: optional method of feature scaling. Default Min-max in range of [0, 1]
    '''
...
class Crop():
    ...
    Cropping unecessary features of image data. Removing non key information from the data item
...
class Resize():
    ...
    Resizing (downsizing, upsizing) of image data.
    
    sample: image data
    shape: tuple, shape of image data
    '''
...
class GaussianBlur():
    ...
    Applying Gaussian filter to reduce downsizing and motion effects
...
class SkullStrip():
    ...
    Remove the skull from the brain image data. If no skull found,
...
class Registration():
    ...
    Spatially align image data to a static image (selected manually).
    
    sample: image data
    static_path: static(template) image path
    '''
